...
def copy_sic(param, chrom_coord, clicks1, link_key, clicks2, time_delay):
    ...
    for i in range(min(len(param), 1)):
        enter_num(param[i])
    for i in range(min(len(param), 2), len(param)):
...
def plot_data(df, x_header, y_headers, x_label, y_label, output_file, output_suf, legend=True):  # Plots the Normalized dataset
    ...
    # plt.show()
    plt.savefig(output_file + output_suf + '.png', dpi=300)  # Save the figure, dpi specifies size.
    return
...
def save_excel(df, sheet_names, output_file):
    ...
    while data_store_try is False:
        try:
            ...
            output_file = input(
                'Error! Cannot save file into a non-existent folder. Input correct file location (do not use r\'\'). \n')
...
def add_excel_img(output_file, output_suf, sheet_name, pic_stag_num):
    img = openpyxl.drawing.image.Image(output_file + output_suf + '.png')  # Opens previously made norm plot, to be inserted in Excel.
